...
def process_revenue(orders_raw, calls_raw):
    ...
    orders['Created at'] = orders['Created at'].str.replace(r'\s\+\d{4}$', '', regex=True)
    orders['Order Time'] = pd.to_datetime(orders['Created at'])
    orders['Order Date'] = orders['Order Time'].dt.strftime('%d-%m-%Y')
    orders['Order Time Str'] = orders['Order Time'].dt.strftime('%d-%m-%Y %H:%M:%S')
    
    # Phone Duplicate Logic
    orders['B_Phone'] = orders['Billing Phone'].apply(clean_phone)
    orders['S_Phone'] = orders['Shipping Phone'].apply(clean_phone)
    
    expanded_rows = []
    for _, row in orders.iterrows():
        channel = "Store" if "retail_store" in str(row['Tags']).lower() else "Website"
        phones = {p for p in [row['B_Phone'], row['S_Phone']] if p}
        for ph in phones:
            expanded_rows.append({
                'Order ID': row['Name'], 'Order Value': float(row['Total']),
                'Order Date': row['Order Date'], 'Order Time': row['Order Time'],
    ...
    calls['Call Time'] = pd.to_datetime(calls['Call Time'], dayfirst=True)
    calls['Phone'] = calls['Phone Number'].apply(clean_phone)
    calls['Talk_Sec'] = calls['User Talk Time'].apply(hms_to_sec)
    calls = calls[calls['Talk_Sec'] >= 1].dropna(subset=['Phone'])

    # 3. ATTRIBUTION
    final_data = []
    for _, order in orders_df.iterrows():
        # Match calls before order time
        match_calls = calls[(calls['Phone'] == order['Order Phone']) & 
                           (calls['Call Time'] < order['Order Time'])]
        ...
        agent_stats = match_calls.groupby('User ID')['Talk_Sec'].sum().reset_index()
        
        # Scenario B: >= 100,000 (Proportionate Split, 3min criteria)
        if order['Order Value'] >= 100000:
            ...
            if qualified.empty:
                ...
                for _, ag in qualified.iterrows():
                    ...
                    final_data.append({**order, 'Agent': ag['User ID'], 'Attributed Revenue': round(order['Order Value'] * share, 2), 'Total Talk Time': sec_to_hms(ag['Talk_Sec'])})
        
        # Scenario A: < 100,000 (Multi-Credit, 1min criteria)
        else:
            ...
            if qualified.empty:
                ...
                for _, ag in qualified.iterrows():
                    final_data.append({**order, 'Agent': ag['User ID'], 'Attributed Revenue': order['Order Value'], 'Total Talk Time': sec_to_hms(ag['Talk_Sec'])})
...
st.markdown("Upload Shopify raw orders and the 4-month call attribution file.")

with st.sidebar:
    st.header("Upload Files")
    order_file = st.file_uploader("1. Shopify Orders Raw", type="csv")
    call_file = st.file_uploader("2. Call Attribution Data (4 Months)", type="csv")
...
if order_file and call_file:
    try:
        ...
        with st.spinner("Processing Attribution..."):
            result_df = process_revenue(o_raw, c_raw)
            # Remove helper col for display
        ...
        # Excel Export with Arial 11 and Yellow Headers
        output = io.BytesIO()
        with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
            ...
            worksheet.set_column(0, len(display_df.columns)-1, 20)
            
        st.download_button("📥 Download Final Revenue Report", data=output.getvalue(), file_name="Revenue_Attribution_Master.xlsx")
